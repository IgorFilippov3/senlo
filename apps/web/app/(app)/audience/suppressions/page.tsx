...
import { useSuppressions, useDeleteSuppression } from "apps/web/queries/suppressions";
import { useState } from "react";
import { format } from "date-fns";

export default function SuppressionListPage() {
  const { data: suppressions = [], isLoading } = useSuppressions();
  const { mutate: deleteSuppression, isPending: isDeleting } = useDeleteSuppression();
  ...
  const handleDelete = (id: number) => {
    if (confirm("Are you sure you want to remove this email from the suppression list? This will allow future emails to be sent to this address.")) {
        ...
        <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden shadow-sm">
          {isLoading ? (
            ...
            <div className="p-12 text-center text-zinc-500 bg-zinc-50/30">
              ...
              <p className="text-zinc-600 font-medium">Your suppression list is clean</p>
              <p className="text-sm mt-1">No email addresses have been blocked yet.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="bg-zinc-50/80 border-b border-zinc-200">
                    <th className="px-6 py-4 font-semibold text-zinc-700">Email Address</th>
                    <th className="px-6 py-4 font-semibold text-zinc-700">Project</th>
                    <th className="px-6 py-4 font-semibold text-zinc-700">Reason</th>
                    <th className="px-6 py-4 font-semibold text-zinc-700">Blocked At</th>
                ...
                <tbody className="divide-y divide-zinc-100">
                  {filteredSuppressions.map((item) => (
                    <tr key={item.id} className="hover:bg-zinc-50/50 transition-colors group">
                      <td className="px-6 py-4 font-medium text-zinc-900">{item.email}</td>
                      <td className="px-6 py-4">
                        <span className="text-zinc-600 px-2 py-1 bg-zinc-100 rounded text-xs font-medium">
                          {item.projectName}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <Badge variant={item.reason === "SPAM" ? "error" : "warning"}>
                          {item.reason}
                        </Badge>
                      </td>
