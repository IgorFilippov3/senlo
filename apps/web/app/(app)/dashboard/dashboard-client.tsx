...
import { getDashboardStats, getDashboardActivity, getRecentEvents } from "./actions";
      ...
      <div className="bg-white p-3 border border-zinc-200 shadow-lg rounded-lg text-xs">
        ...
        <div className="space-y-1">
          <p className="flex items-center gap-2">
            ...
            <span className="font-bold text-zinc-900">{payload[0]?.value ?? 0}</span>
          </p>
          <p className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-rose-500" />
            <span className="text-zinc-500">Suppressed:</span>
            <span className="font-bold text-zinc-900">{payload[1]?.value ?? 0}</span>
          </p>
        </div>
      </div>
    );
...
export function DashboardClient() {
  ...
  const openRate = stats?.totalSent ? ((stats.opened / stats.totalSent) * 100).toFixed(1) : "0.0";
  const clickRate = stats?.totalSent ? ((stats.clicked / stats.totalSent) * 100).toFixed(1) : "0.0";
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Overview of your email infrastructure performance.</p>
        </div>
        ...
        <Card className="p-4 border-zinc-200 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-bold text-zinc-900">{openRate}%</span>
              <span className="text-xs text-zinc-400">{stats?.opened} unique opens</span>
            </div>
          </div>
        </Card>
        <Card className="p-4 border-zinc-200 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-zinc-500">Average Click Rate</p>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-bold text-zinc-900">{clickRate}%</span>
              <span className="text-xs text-zinc-400">{stats?.clicked} unique clicks</span>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-2 p-6 border-zinc-200">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={activity} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
        ...
        <Card className="p-6 border-zinc-200">
          ...
          <div className="space-y-6">
            {events.length === 0 ? (
              <p className="text-sm text-zinc-500 text-center py-8">No recent activity detected.</p>
            ) : (
              events.map((event) => (
                <div key={event.id} className="flex gap-3 relative">
                  <div className="mt-1 shrink-0">
                    ...
                    ) : event.type === "BOUNCE" || event.type === "SPAM" || event.type === "FAILED" ? (
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-zinc-500 line-clamp-2 mt-0.5">{event.description}</p>
